...
export class AppMenu {
    ...
    ngOnInit() {
        const fullModel: MenuItem[] = [
            {
                label: 'Home',
                items: [{ label: 'Dashboard', icon: 'pi pi-fw pi-home', routerLink: ['/dashboard'], data: { roles: [RoleEnum.ADMIN, RoleEnum.USER] } }]
            },
            {
                label: 'UI Components',
        ...
        this.model = this.filterMenu(fullModel);
    }
    ...
    private filterMenu(items: MenuItem[]): MenuItem[] {
        ...
        return items
            .filter((item: any) => {
                const roles = item.data?.roles;
                if (!roles) return true;
                return userRoleId && roles.includes(userRoleId);
            })
            .map((item: MenuItem) => {
                if (item.items) {
